...
main() {
  ...
  # so load_secrets()'s permission check never runs for it either -- check
  # independently and refuse to proceed, since this is often the first/only
  # tool run on a freshly-provisioned host.
